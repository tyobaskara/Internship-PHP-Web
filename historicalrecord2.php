...
<?php
			...
			{
				$result2 = mysql_query("Select count(*) From snhr where PART_NUMBE like '%".$_GET['search']."%'");
			}
			...
			{
				$result = mysql_query("Select * From snhr where PART_NUMBE like '%".$_GET['search']."%'limit $start,$dataPerPage");
			}
			...
			for($i=1;$i<=$jumlahPage;$i++)
				echo "<a href='historicalrecord2.php?search=".$_GET['search']."&page=$i'>$i</a> &emsp;";
		?>
</div>
</body>
</html>